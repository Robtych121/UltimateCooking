...
def manage_cuisines():
    return render_template("cuisine/cuisine_manage.html",
                           cuisines=mongo.db.cuisines.find())


@app.route('/add_cuisine')
def add_cuisine():
    return render_template("cuisine/cuisine_add.html")
...
def insert_cuisine():
    ...
    upload_file(app.config['UPLOAD_FOLDER_CUISINE'] + "/" + f.filename,
...
def edit_cuisine(cuisine_id):
    ...
    return render_template("cuisine/cuisine_edit.html",
                           cuisine=the_cuisine,
                           imagePath=app.config['UPLOAD_FOLDER_CUISINE'],
...
def update_cuisine(cuisine_id):
    ...
    pictures = mongo.db.cuisines.find_one({'_id': ObjectId(cuisine_id)},
                                          {'picture': 1, '_id': 0})
...
def edit_cuisine_picture(cuisine_id):
    the_cuisine = mongo.db.cuisines.find_one({'_id': ObjectId(cuisine_id)})
    return render_template("cuisine/cuisine_pic_edit.html",
                           cuisine=the_cuisine,
                           imagePath=app.config['UPLOAD_FOLDER_CUISINE'],
                           s3link=app.config['AWS_BUCKET_LINK'])
...
def view_cuisines_grid():
    return render_template("cuisine/cuisine_view_grid.html",
                           cuisines=mongo.db.cuisines.find(),
                           imagePath=app.config['UPLOAD_FOLDER_CUISINE'],
                           s3link=app.config['AWS_BUCKET_LINK'])
